(function($, undefined) {
    ...
    $.widget("ech.multiselect", {
        ...
        _buildButton: function() {
            ...
            var $button = (this.$button = $(document.createElement("button"))).addClass("ui-multiselect ui-widget ui-state-default ui-corner-all" + (wrapText.indexOf("button") > -1 ? "" : " ui-multiselect-nowrap") + (this.options.classes ? " " + this.options.classes : "")).attr({
                type: "button",
                title: this.element[0].title,
                tabIndex: this.element[0].tabIndex,
                id: this.element[0].id ? this.element[0].id + "_ms" : null
            }).prop("aria-haspopup", true).html(this._linkHTML('<span class="{{class}}" title="{{title}}">{{icon}}</span>', "open"));
            this.$buttonlabel = $(document.createElement("span")).html(this.options.noneSelectedText || this.element[0].placeholder).appendTo($button);
            return $button
        },
        ...
        _create: function() {
            ...
            this.$headerLinkContainer = $(document.createElement("ul")).addClass("ui-helper-reset").html(this._buildHeaderHtml() + (!options.listbox ? this._linkHTML('<li class="{{class}}"><a class="{{class}}" title="{{title}}">{{icon}}</a></li>', "close") : ""));
            var $header = (this.$header = $(document.createElement("div"))).addClass("ui-multiselect-header ui-widget-header ui-corner-all ui-helper-clearfix").append(this.$headerLinkContainer);
            ...
            var $menu = (this.$menu = $(document.createElement("div"))).addClass("ui-multiselect-menu ui-widget ui-widget-content ui-corner-all" + ($element[0].multiple ? "" : " ui-multiselect-single") + (!options.listbox ? "" : " ui-multiselect-listbox") + (this.options.classes ? " " + this.options.classes : "")).append($header, $checkboxes);
            if (!options.listbox) {
        ...
        _linkHTML: function(linkTemplate, linkID) {
            var self = this;
            return linkTemplate.replace(/{{(.*?)}}/gi, function(m, p1) {
        ...
        _makeOption: function(option) {
            ...
            item.className = (isDisabled ? "ui-multiselect-disabled " : "") + (this.options.groupColumns ? " ui-multiselect-columns" : "") + (option.className || "");
        ...
        _buildOptionList: function() {
            ...
            this.element.children().each(function() {
                ...
                if (elem.tagName.toUpperCase() === "OPTGROUP") {
                    ...
                    var $collapseButton = !!self.options.groupsCollapsable ? $(document.createElement("button")).attr({
                        title: self.linkInfo.collapse.title,
                        type: "button"
                    }).addClass("ui-state-default ui-corner-all ui-multiselect-collapser").html(self.linkInfo.collapse.icon) : null;
                    var $optGroupLabel = $(document.createElement("a")).addClass("ui-multiselect-grouplabel" + (self.options.groupsSelectable ? " ui-multiselect-selectable" : "")).html(elem.getAttribute("label"));
                    var $optionGroup = $(document.createElement("ul")).append(options);
                    var $optGroupItem = $(document.createElement("li")).addClass("ui-multiselect-optgroup" + (self.options.groupColumns ? " ui-multiselect-columns" : "") + (elem.className ? " " + elem.className : "")).append($collapseButton, $optGroupLabel, $optionGroup);
        ...
        _bindButtonEvents: function() {
            ...
            }).find("span").on("click.multiselect,click", buttonClickHandler)
        },
        ...
        _bindCheckboxEvents: function() {
            ...
            self.$checkboxes.on("click.multiselect", ".ui-multiselect-grouplabel", function(e) {
                ...
                if (self._trigger("beforecollapsetoggle", e, {
                    label: optgroupLabel,
                ...
                if (input.disabled || self._trigger("click", e, {
                    value: val,
                    text: optionText,
                    checked: checked
                }) === false) {
                ...
                if (maxSelected && checked && numChecked > maxSelected) {
                    if (self._trigger("maxselected", e, {
                        labels: self.$labels,
                        inputs: $allInputs
                    }) !== false) {
        ...
        _bindHeaderEvents: function() {
            var self = this;
            self.$header.on("click.multiselect", "a", function(e) {
                var headerLinks = {
        ...
        _bindEvents: function() {
            ...
            this._setResizable();
            this.document.on("mousedown" + this._namespaceID + " wheel" + this._namespaceID + " mousewheel" + this._namespaceID, function(event) {
        ...
        _traverse: function(which, start) {
            ...
            if (!$next.length) {
                ...
                $container.find("label").filter(":visible")[moveToLast ? "last" : "first"]().trigger("mouseover", {
                    allowScroll: true
                });
    ...
    $.widget("ech.multiselectfilter", {
        ...
        _create: function() {
            ...
            this.$input = $(document.createElement("input")).attr({
                ...
                width: typeof opts.width === "string" ? this.instance._parse2px(opts.width, this.$header).px + "px" : /\d/.test(opts.width) ? opts.width + "px" : null
            });
